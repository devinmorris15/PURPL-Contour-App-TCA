...
with st.sidebar:
	...
	if prop_choices == "Simple Propellants List":
		...
		with col2:
			fuel = st.selectbox("Fuel", ["RP_1", "LH2", "CH4"], index=0) #Choose Fuel Select Box
	else:
		with col1:
			ox = st.selectbox("Oxidizer", ["90_H2O2", "98_H2O2","LOX", "N2O"], index=0) #Choose Oxidizer Select Box
		with col2:
			fuel = st.selectbox("Fuel", ["RP_1", "GH2"], index=0) #Choose Fuel Select Box
...
with tab4:
	...
	if st.session_state.get("computed"):
		...
		with col1:
			chamb_mat = st.selectbox("Choose your chamber/nozzle material:", ["Stainless Steel 304", "Terillium Copper 145", "Copper Nickel 715"])
		with col2:
			h_natural = st.number_input("Natural Convection Coefficient (W/m^2-K)", min_value = 0.0, max_value = 1000.0, value = 5.0, key = "h_natty", help = "5 is low value, 25 is a high value")

		col1, col2, col3 = st.columns(3)
		with col1:
			init_temp = st.number_input("Initial Temperature", min_value = -460.0, max_value = 10000.0, value = 298.0, key = "i_temp1d")
		with col3:
			temp_units = st.selectbox("Temperature Units", ["°C", "°F", "°K", "°R"], index = 2)
		with col2:
			amb_temp = st.number_input("Ambient Temperature", min_value = -460.0, max_value = 10000.0, value = 298.0, key = "amb_temp1d")
		
		col1, col2, col3, col4 = st.columns([3,3,3,1.5])
		with col1:
			run_time = st.number_input("Sim Run Time (s)", min_value = 0.0, max_value = 100.0, value = 5.0, key = "runtime")
		with col2:
			time_int = st.number_input("Sim Time Step (s)", min_value = 0.0, max_value = run_time, value = 0.0005, key = "time_int", format="%.5f")
		with col3:
			nodes = st.number_input("Wall Res. (#nodes)", min_value = 0.0, max_value = 1000.0, value = 100.0, key = "nodes_sim", help="Number of calculation points through the wall. Higher = more accurate but slower.")
		with col4:
			fps = st.number_input("FPS", min_value = 0, max_value = 60, value = 30, key = "fps", help = "Simulation Frame rate per second")

		col1, col2, col3 = st.columns(3)
		with col1:
			cwall = st.number_input("Wall Thickness at Chamber", min_value = 0.0, max_value = 10000.0, value = 0.0, key = "cwall_t", format="%.3f")
		with col3:
			wall_units = st.selectbox("Thickness Units", ["in", "mm", "cm", "m"], index = 0)
		with col2:
			twall = st.number_input("Wall Thickness at Throat", min_value = 0.0, max_value = 10000.0, value = 0.0, key = "twall_t", format="%.3f")
		...
		with col1:
			run_1dcsims = st.button("Run Chamber Heat Transfer Sim", type="primary", use_container_width=True, key="run_csims_btn")
			if run_1dcsims == True:
				if not cwall or cwall == 0:
					st.error("Chamber wall thickness is required to run chamber analysis.")
				else:
					st.session_state.thermal_mode = "chamber"
					st.session_state.pop("thermal_output", None)
		with col2:
			run_1dtsims = st.button("Run Throat Heat Transfer Sim", type="primary", use_container_width=True, key="run_tsims_btn")
			if run_1dtsims == True:
				if not twall or twall == 0:
					st.error("Throat wall thickness is required to run throat analysis.")
				else:
					st.session_state.thermal_mode = "throat"
					st.session_state.pop("thermal_output", None)
		with col3:
			run_1dbsims = st.button("Run Together", type="primary", use_container_width=True, key="run_bsims_btn")
			if run_1dbsims == True:
				if not cwall or cwall == 0:
					st.error("Chamber wall thickness is required to run full analysis.")
				elif not twall or twall == 0:
					st.error("Throat wall thickness is required to run full analysis.")
				else:
					st.session_state.thermal_mode = "both"
					st.session_state.pop("thermal_output", None)
		
		if chamb_mat == "Stainless Steel 304":
			material = mats["SS304"]
		elif chamb_mat == "Terillium Copper 145":
			material = mats["C145"]
		elif chamb_mat == "Copper Nickel 715":
			material = mats["C715"]

		if any([run_1dcsims, run_1dtsims, run_1dbsims]) and "thermal_mode" in st.session_state:
			with st.spinner("Running thermal simulation..."):
				cea_cfg= {
					"Pc_chamber_psia": pc_psi,
					"MR": of_ratio,
					"oxName": ox,
					"fuelName": fuel,
					"eps": Eratio,
				...
				if temp_units == "°K":
					...
					amb_temp = amb_temp + celciusToKelvin
					init_temp = init_temp + celciusToKelvin
				elif temp_units == "°F":
					amb_temp = (amb_temp - rankineToF) * rankineToKelvin
					init_temp = (init_temp - rankineToF) * rankineToKelvin
				elif temp_units == "°R":
					amb_temp = amb_temp * rankineToKelvin
					init_temp = init_temp * rankineToKelvin

				if wall_units == "in":
					cwall = cwall
					twall = twall
				elif wall_units == "cm":
					cwall = cwall / in_to_cm
					twall = twall / in_to_cm
				elif wall_units == "mm":
					cwall = cwall / in_to_mm
					twall = twall / in_to_mm
				elif wall_units == "m":
					cwall = cwall / in_to_m
					twall = twall / in_to_m
				...
				chamber_geom = {
				"station": "chamber_section",
				"D_in": Dc_m / in_to_m,      # inner diameter of chamber in inches
				"Throat_D_in": (Dt_o) / (conv_o * in_to_m), # throat diameter in inches
				"L_in": cwall  # wall thickness at chamber in inches
				}

				throat_geom = {
					"station": "throat_section",
					"D_in": (Dt_o) / (conv_o * in_to_m),        # same as Throat_D_in at throat
					"Throat_D_in": (Dt_o) / (conv_o * in_to_m),
		...
		st.info("Run the solver to prepare 1-D heat simulator.")
